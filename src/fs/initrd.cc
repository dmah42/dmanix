...
// TODO(dominic): dynamic array type
fs::Node* nodes;
uint32_t num_nodes;
...
fs::Node* Initialize(const multiboot::Module& initrd_module) {
  ...
  root = new (root_mem) fs::Node("initrd", fs::FLAG_DIRECTORY);  // NOLINT
  root->readdir = ReadDir;
  root->finddir = FindDir;

  void* dev_mem = kalloc(sizeof(fs::Node));
  dev = new (dev_mem) fs::Node(DEV, fs::FLAG_DIRECTORY);  // NOLINT
  dev->readdir = ReadDir;
  dev->finddir = FindDir;

  // Add the files
  header = reinterpret_cast<Header*>(location);
  file_headers = reinterpret_cast<FileHeader*>(location + sizeof(Header));

  num_nodes = header->num_files;
  // screen::Printf("  num_files: %u\n", num_nodes);

  void* nodes_mem = (fs::Node*) kalloc(sizeof(fs::Node) * num_nodes);
  nodes = new (nodes_mem) fs::Node[num_nodes]();  // NOLINT
  for (uint32_t i = 0; i < num_nodes; ++i) {
